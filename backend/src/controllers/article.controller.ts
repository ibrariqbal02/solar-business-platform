 ...
 *         tags (JSON array), status, publishedAt
 * File: featuredImage
 */
export const createArticle = async (req: Request, res: Response): Promise<void> => {
  try {
    const { title, category, excerpt, description, technicalExplanation, safetyInformation, status, publishedAt } = req.body;
    ...
    if (relatedVideoIds.length > 0) {
      const found = await Video.countDocuments({ _id: { $in: relatedVideoIds } });
      if (found !== relatedVideoIds.length) {
    ...
    if (relatedProductIds.length > 0) {
      const found = await Product.countDocuments({ _id: { $in: relatedProductIds } });
      if (found !== relatedProductIds.length) {
    ...
    const articlePublishedAt =
      articleStatus === "published" ? (publishedAt ? new Date(publishedAt) : new Date()) : undefined;
...
/**
 * GET /api/articles
 * Query: search, category, status, sort (newest|oldest|published), page, limit
 */
 ...
 * PUT /api/articles/:id
 * Content-Type: multipart/form-data
 * All fields optional. Pass removeFeaturedImage="true" to clear image.
 */
export const updateArticle = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { title, category, excerpt, description, technicalExplanation, safetyInformation, status, publishedAt, removeFeaturedImage } = req.body;
    ...
    article.readTimeMinutes = estimateReadTime([article.description, article.technicalExplanation, article.safetyInformation]);
...
/**
 * DELETE /api/articles/:id  — sets status = "unpublished"
 */
export const deleteArticle = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(200).json({ success: true, message: "Article unpublished successfully", data: article });
  } catch (error: any) {
