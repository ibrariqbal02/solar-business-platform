...
import { articleCategoriesApi } from '../../../api/articles.api';
import type { AdminArticleCategoriesQuery } from '../../../api/articles.api';
...
function FormPanel({ mode, initial, onClose, onSaved }: FormPanelProps) {
  ...
  const onSubmit = async (values: CatFormValues) => {
    ...
    try {
      if (mode === 'create') {
        await articleCategoriesApi.create(values);
      } else {
        await articleCategoriesApi.update(initial!._id, values);
      }
      onSaved();
    } catch (err: unknown) {
      const msg =
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-gray-700">
            {mode === 'create' ? 'Add New Article Category' : `Edit: ${initial?.name}`}
          </h2>
          <button
            type="button"
            onClick={onClose}
...
export default function ArticleCategoriesPage() {
  ...
  const [editTarget, setEditTarget]       = useState<ArticleCategory | null>(null);
  ...
  const queryParams: AdminArticleCategoriesQuery = {
  ...
  const { data: response, isLoading, isError, isFetching } = useQuery({
    ...
    queryFn: async () => {
      const res = await articleCategoriesApi.adminGetAll(queryParams);
      return res.data;
    },
  ...
  const categories: ArticleCategory[]         = response?.data ?? [];
  ...
  const handleSaved = async () => { await invalidate(); setFormMode(null); setEditTarget(null); };

  const handleToggleStatus = async (cat: ArticleCategory) => {
    setRowLoading(cat._id, true);
    try {
      await articleCategoriesApi.toggleStatus(cat._id, !cat.isActive);
      await invalidate();
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await articleCategoriesApi.delete(deleteTarget.id);
      await invalidate();
    } finally {
  ...
  return (
    <div className="py-6">
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1 min-w-0">
          ...
          <input
            ...
            aria-label="Search article categories"
      ...
      <div className="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden">
        ...
        {isError && <div className="px-6 py-8 text-center text-sm text-red-600">Failed to load categories.</div>}
      ...
      <ConfirmationDialog
        ...
        message={deleteTarget ? `"${deleteTarget.name}" will be deactivated. Existing articles will remain but won't be filterable by it. You can restore it using the toggle.` : ''}
        confirmLabel="Delete"
        cancelLabel="Cancel"
